...
parser.add_argument('-I', '--input', required=True, help='Please indicate the pathway to input .bed file created with "Fst_Z_score_transformer".')
parser.add_argument('-f', '--frequency', required=True, help="Indicate pathway to _pwc delta allele frequency file provided from Popoolation.")
parser.add_argument('-r', '--reference', required=True, help='Provide pathway to reference .gff or .gtf file.')
parser.add_argument('-t', '--threshold', required=True, help='Indicate the minimum transformed Z-score used in Gene identification.')
parser.add_argument('-c', '--chromosome', required=True, help="'all' = every chromosome,i.e 'NC_009144.3, NC_009145.3' = graphs for indicated chromosomes, or give name for singular chromosome. ")
    ...
    def datasetup(chrom):
        ...
        for j in data_df['Chromosome']:
            if str(chrom) in str(j):
                ...
                holder2 += 1
                continue
        for j2 in freq_df['##chr']:
            if str(chrom) in str(j2):
