...
def main(region, vcf_file, output):
    ...
    reg_file = pd.read_csv(region, sep='\t', dtype={"Region": int, "Chromosome": str, "Start": int, "End": int,
                                                    "Genes": str, "Max_Fst": float, "Max_Zscore": float,
                                                    "Max_Position": int})
    ...
    for i3 in range(0, len(reg_file["Region"])):
        ...
        wanted_name2 = (wanted + "/" + "REG" + str(reg1) + "_" + str(chrom[i3]) + '_'
                        + str(start1[i3]) + '_' + str(end1[i3]) + '.ANN.vcf')
        other_name2 = (other + "/" + "REG" + str(reg1) + "_" + str(chrom[i3]) + '_'
                       + str(start1[i3]) + '_' + str(end1[i3]) + '.ANN.vcf')
        ...
        try:
            for record in vcf_reader2.fetch(chrom[i3], start1[i3], end1[i3]):
                ...
                if 'missense_variant' in comp_string:
...
def region_number_fix():
    reg_read = pd.read_csv('tmp/temp_region.txt', delimiter='\t')
    ...
    for char in reg_nums:
        if char == current_number:
            ...
            placement += 1
            continue
    reg_df.to_csv('tmp/temp_region_final.txt', sep='\t', index=False)
    return
...
def na_check(orig_region):
    ...
    with open(orig_region, 'r') as f:
        with open('tmp/temp_region.txt', 'w') as output_file:
            for line in f:
                if 'na' in line:
                    trip += 1
                    continue
...
def file_clean(pathways):
    for folder in pathways:
        ...
        for file in folder_list:
            ...
            check_total = 0
            header_total = 0
            with open(full_path, 'r') as curr_open:
                for line in curr_open:
                    if '#' in line:
                        header_total += 1
                        check_total += 1
                        continue
...
if __name__ == '__main__':
    ...
    Main_Call = main('tmp/temp_region_final.txt', args.input, args.output)
    regulator += 1
    if regulator == 1:
        file_clean(Main_Call)
